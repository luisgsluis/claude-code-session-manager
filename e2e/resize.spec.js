// CCSM e2e for pane auto-resize: the Terminal tab and each grid tile must
// tell the backend how many columns/rows their pane actually renders, so
// Claude Code (a normal TUI that just wraps to the real terminal geometry)
// wraps its output to that width instead of tmux's default 80x24 — these
// sessions run detached and no real terminal client ever attaches to report
// a size on its own. See Host.sessionResize and requestPaneResize (app.js).
...
test('live terminal tab resizes the tmux window to fit the pane', async ({ page }) => {
  ...
  const resizeReq = page.waitForResponse(
    (r) => r.url().includes('/api/sessions/resize-tab/resize') && r.request().method() === 'POST'
  );
